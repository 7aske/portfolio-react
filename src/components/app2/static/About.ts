...
import { contactFmt, eduFmt, fold } from "../utils/Utils";
...
# variable below
#
/*ANCHOR[%RESUME%,%${resume}%]*/="$(curl ${window.location.protocol}//${window.location.hostname}${resume})" 

# past education
declare -A EDU
${aboutEducation.map(edu => eduFmt(edu, "bash")).join("\n")}

# contact information
declare -a CONTACT
${aboutContact.map((c,i) => `CONTACT[${i}]=`+contactFmt(c, "bash")).join("\n")}
